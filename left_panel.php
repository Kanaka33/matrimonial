...
<div class="sidebar sidebar-inverse sidebar--style-1 bg-base-1 z-depth-2-top">
    ...
    <?php } ?>
        ...
        <div class="profile-stats clearfix mt-2">
            ...
            <div class="stats-entry">
                <span class="stats-label text-uppercase text-left pl-2"> <?=$calculated_age = (date('Y') - date('Y', strtotime($get_member[0]->date_of_birth)));?></span>
                <span class="stats-label text-uppercase text-left pl-2"><?=$this->Crud_model->get_type_name_by_id('language', $language_data[0]['mother_tongue']);?></span>
                <span class="stats-label text-uppercase text-left pl-2"><?php echo translate('hinduism');?></span>
                <span class="stats-label text-uppercase text-left pl-2"><?php echo translate('kamma');?></span>
                <span class="stats-label text-uppercase text-left pl-2"><?=$get_member[0]->height." ".translate('feet')?></span>
                <span class="stats-label text-uppercase text-left pl-2"><?php if($present_address_data[0]['country']){echo $this->Crud_model->get_type_name_by_id('state', $present_address_data[0]['state']).', '.$this->Crud_model->get_type_name_by_id('country', $present_address_data[0]['country']);}?></span>
            </div>
        </div>
